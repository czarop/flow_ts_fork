...
pub fn generate_mixing_matrix(n_detectors: usize, n_endmembers: usize) -> Array2<f64> {
    ...
    for i in 0..n_detectors {
        for j in 0..n_endmembers {
            ...
            if i == j {
                matrix[(i, j)] = 0.8 + rng.gen_range(0.0..0.2);
            } else {
                matrix[(i, j)] = rng.gen_range(0.0..0.1);
            }
        }
    }
    ...
    for i in 0..n_events {
        for j in 0..n_endmembers {
            if rng.gen::<f64>() > sparsity {
                abundances[(i, j)] = rng.gen_range(0.0..100.0);
            }
        }
    }
...
mod tests {
    ...
    fn test_tru_ols_synthetic() {
        ...
        // Unmix
        let unmixed = tru_ols.unmix(&observations.t()).unwrap();
